#!/usr/bin/env python3
"""
Mid Cap Mutual Fund Scoring Algorithm - Codex (Research-Led)

This model is purpose-built for Indian Mid Cap funds and tuned specifically for
forward 1-year excess return prediction (vs Mid Cap benchmark).

Design choices are based on common institutional MF evaluation patterns:
- Persistence and active-return quality (alpha, IR, rolling stability)
- Capture behavior across up/down and stress regimes
- Drawdown pain and recovery speed
- Tempered momentum (with overheat penalty)
- 3Y/5Y compounding anchors

Process:
1) Build a historical cross-sectional factor panel at many past dates.
2) Backtest each date against next-1Y realized excess return.
3) Tune weights using walk-forward cross-validation.
4) Apply a small market-regime tilt to the final live ranking.

Outputs:
- results/Mid Cap_Codex.csv
- data/tmp/Mid Cap_Codex_weights.csv
- data/tmp/Mid Cap_Codex_backtest.csv
...
BENCHMARK_INDEX = "Mid Cap"  # resolves to .NIMI150 in MfDataProvider
...
# Tuning controls
SEARCH_TRIALS = 140
REFINE_MUTATIONS_PER_BASE = 1
TOP_TRIALS_FOR_ENSEMBLE = 6
RANDOM_SEED = 63
MAX_FACTOR_WEIGHT = 0.24
MAX_CV_FOLDS = 2
...
FACTOR_SPECS: List[Tuple[str, bool]] = [
    # --- Skill and active return quality ---
    ("alpha_1y", True),
    ("info_1y", True),
    ("sortino_1y", True),
    ...
    # --- Market-capture and regime behavior ---
    ("up_capture_1y", True),
...
def recovery_speed_score(nav_window: pd.Series, threshold: float = -0.05) -> float:
    ...
    if not ratios:
        # If no completed recoveries, give a conservative low score.
        return 0.2
...
def rolling_alpha_stability(
    ...
    # Bounded blend for cross-sectional ranking stability.
...
def is_weight_vector_valid(weights: np.ndarray) -> bool:
    ...
    regime = group_weight_sum(weights, REGIME_FACTORS)
    downside = group_weight_sum(weights, DOWNSIDE_FACTORS)
    momentum = group_weight_sum(weights, MOMENTUM_FACTORS)
    anchors = group_weight_sum(weights, ANCHOR_FACTORS)

    if not (0.22 <= skill <= 0.55):
        return False
    if not (0.18 <= regime <= 0.40):
        return False
    if not (0.16 <= downside <= 0.38):
        return False
    if not (0.08 <= momentum <= 0.26):
        return False
    if not (0.07 <= anchors <= 0.22):
        return False

    # Ensure minimum diversification.
    if int((weights >= 0.03).sum()) < 6:
        return False

    if weight_entropy(weights) < 0.72:
        return False
...
def build_historical_panel(funds: List[Dict[str, Any]], bench_nav: pd.Series) -> pd.DataFrame:
    ...
    for idx in range(eval_start_idx, eval_end_idx, EVAL_STEP_WEEKS):
        ...
        for fund in funds:
            ...
            factor_row = build_feature_row(nav, bench_nav, idx)
            if not factor_row:
                continue
...
def evaluate_segment(
    ...
    objective = (
        0.52 * (mean_top_excess * 100.0)
        + 0.18 * (mean_ic * 100.0)
        + 0.12 * ((hit_rate - 0.5) * 100.0)
        + 0.08 * (mean_top_return * 100.0)
        - 0.11 * (downside_excess * 100.0)
        ...
        vec(
            {
                ...
                "info_1y": 0.09,
                "sortino_1y": 0.08,
                "rolling_alpha_stability_2y": 0.08,
                "rolling_active_ir_2y": 0.06,
                "up_capture_1y": 0.06,
                "down_capture_1y": 0.08,
                "capture_spread_1y": 0.06,
                "bear_excess_1y": 0.06,
        ...
        vec(
            {
                ...
                "capture_spread_1y": 0.07,
                "bear_excess_1y": 0.05,
                "high_vol_excess_1y": 0.04,
                "max_drawdown_2y": 0.06,
                "ulcer_2y": 0.06,
                "recovery_speed_2y": 0.05,
                "rolling_beat_rate_2y": 0.05,
                "momentum_6m_rel": 0.07,
                "momentum_12_1_rel": 0.06,
...
def tune_weights(cv_folds: List[Dict[str, Any]]) -> Tuple[np.ndarray, pd.DataFrame]:
    ...
    for w in candidates:
        ...
        score = m_va["objective"] + 0.15 * m_tr["objective"] + 0.20 * weight_entropy(w)
        screening_scores.append(score)

    pre_top = min(len(candidates), max(20, SEARCH_TRIALS // 7))
    ...
    eligible = trials_df[trials_df["cv_worst_val_top_excess_pct"] > -5.0]
    if eligible.empty:
        eligible = trials_df
...
def detect_market_regime(bench_nav: pd.Series) -> Dict[str, Any]:
    ...
    if pd.notna(ret_6m) and pd.notna(ret_3m):
        if ret_6m > 0.10 and ret_3m > 0 and (pd.isna(vol_ratio) or vol_ratio < 1.05):
            regime = "bull_calm"
        elif ret_6m < -0.03 or ret_3m < -0.05:
            regime = "correction"
...
def apply_regime_tilt(weights: np.ndarray, regime: str) -> np.ndarray:
    ...
    if regime == "bull_calm":
        ...
        for f in DEFENSIVE_TILT_FACTORS:
            multipliers[FACTOR_INDEX[f]] *= 0.92
...
def score_current_snapshot(
    ...
    # Horizon-coverage aware neutralization.
...
def main() -> None:
    ...
    if len(bench_nav) < LB_1Y + FORWARD_HORIZON_WEEKS:
    ...
    df_mid = df_all[df_all["subsector"] == SUBSECTOR].copy()
    print(f"  Funds in subsector: {len(df_mid)}")
    ...
    if not funds:
        raise RuntimeError("No eligible Mid Cap funds available for analysis.")
    ...
    print("\n" + "-" * 84)
    print("  BACKTEST SUMMARY (Top-5 portfolio each snapshot, static tuned weights)")
    print("-" * 84)
    ...
    print("\n" + "-" * 84)
    print("  TOP 15 MID CAP FUNDS (CODEX)")
    print("-" * 84)
    print(
        output.head(15)[
            [
